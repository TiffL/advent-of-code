...
def is_safe(original_report):
    # adjust for tolerance of a single bad level
    for skip in range(len(original_report)):
        report = [original_report[i] for i in range(len(original_report)) if i != skip]
        ...
        if has_same_sign and is_within_range:
            return True
